...
//		Point2D<VT,INVALIDVT> & operator/=(const Point2D<VT,INVALIDVT> &a) { x /= a.x; y /= a.y; invalid = false; return *this; }
	...
	protected: 
		Point2D<VT,INVALIDVT> & operator=(const Vector2D<VT,INVALIDVT> &a);	// 禁止Vector和Point之间隐式转换
	...
	public:
		Vector2D() :Point2D() {}
		explicit Vector2D(const Point2D<VT,INVALIDVT>& point) :Point2D(point) {} // 禁止Vector和Point之间隐式转换
